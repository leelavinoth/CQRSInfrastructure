...
#pragma warning restore 618
            ...
            Action<Exception> exceptionCallback)
        {
            ...
            this.pendingEventsQueueRetryPolicy
                .ExecuteAction(() => query.ExecuteSegmentedAsync(token))
                .ContinueWith(t =>
                {
                    if (t.Exception != null)
                    {
                    ...
                    {
                        successCallback(t.Result.Results, t.IsCompleted);
                    }
        ...
        public IEnumerable<string> GetPartitionsWithPendingEvents()
        {
            ...
            var query = TableQueryableExtensions.AsTableQuery(table.CreateQuery<EventTableServiceEntity>()
                .Where(eventTable => eventTable.RowKey != null && (System.String.Compare(eventTable.RowKey, UnpublishedRowKeyPrefix,
            ...
            //    TableQuerySegment<DynamicTableEntity> segment = await query.ExecuteSegmentedAsync(continuationToken);
            ...
            string maxRowKey)
        {
            ...
            var entityQuery = table.CreateQuery<EventTableServiceEntity>()
                .Where(eventTable => eventTable.RowKey != null && (eventTable.PartitionKey == partitionKey
                                                                   &&
